...
Code changes are event-driven, but **posture, container images, and live web apps drift on their own** — a base image gets a new CVE overnight, an admin loosens an org policy. For those, you re-scan on a schedule. Two ways, use whichever fits:

### In BreachLens (built-in scheduler)

Every asset's **Scan** tab has a **Scheduled scans** section: set a cadence — a cron expression plus an IANA timezone (e.g. `0 2 * * *` in `America/New_York`) — pick the scan types, and BreachLens runs it for you. It works across every tier: nightly DAST or pentest on a domain, weekly Trivy on a container, daily posture on a cloud or GitHub account, and so on.

To apply one cadence to **many** targets at once, open **Settings → Scan schedules → New schedule**: choose a tier, multi-select the assets, set the cadence, and BreachLens creates one schedule per selected asset. That same page lists every cadence org-wide so you can pause or remove them in one place. Scheduler-fired scans are recorded with a `SCHEDULED` trigger type, so an operator re-scan is distinguishable from a PR scan in the history.

<Warning>
  A recurring **aggressive** pentest submits real, *mutating* exploit payloads on every run. BreachLens defaults pentest schedules to **Standard** and requires an explicit opt-in before an aggressive cadence can be saved — point those at staging or a disposable-data account, never production.
</Warning>

### From your own scheduler (CI / cluster)

Prefer to keep the cadence next to the rest of your automation, or run it in an air-gapped install with no browser? Drive scans from your CI or cluster with the [`breachlens` CLI](/cli) — the same `SCHEDULED` trigger type applies. Pick whichever scheduler fits your environment:
...
<Steps>
  ...
  <Step title="Schedule the drifters">
    Add a **Scheduled scan** on each asset — or set one cadence across many at once from **Settings → Scan schedules** — for nightly posture, container, and DAST. Prefer to run it from your own automation? Use a CI job or CronJob with `--severity-gate none`.
  </Step>
