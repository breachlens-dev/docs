...
```
 IDE            COMMIT / PR      BUILD             TEST             DEPLOY / RUN
 shift-left     SAST + secrets   SCA · IaC · SBOM  DAST · Pentest   Runtime · Posture
   │              │                │                 │                │
   ▼              ▼                ▼                 ▼                ▼
 Squigglies +   PR check runs +  Dependency +      Outside-in +     Live detection +
 working-copy   auto-fix PRs     supply-chain      Proof-of-        continuous posture
 scan                            scanning          Exploit          + attack-paths
```
...
| **Build** | SCA, IaC, container CVEs, signed SBOM | Run the CLI in the build job, or let PR checks cover it |
| **Test** | DAST + autonomous pentest with Proof-of-Exploit | Add a [Domain target](mailto:hi@breachlens.app?subject=BreachLens%20DAST%20setup) pointed at your staging URL |
