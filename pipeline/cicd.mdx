...
| **CI (any stage)** | Any tier, gate the build on severity | **Generate a pipeline file** in Settings → Integrations, or copy a [template](https://github.com/breachlens-dev/breachlens/tree/main/ci-templates) — GitHub Actions, GitLab CI, Jenkins |
...
Prefer to copy from source? The canonical [templates](https://github.com/breachlens-dev/breachlens/tree/main/ci-templates) — one per platform — all share the same trigger → poll → SARIF → severity-gate contract:

<CodeGroup>
```yaml GitHub Actions
# .github/workflows/breachlens.yml — wraps the composite action;
# auto-onboards the repo and uploads SARIF to the Security tab.
- uses: breachlens-dev/breachlens/.github/actions/breachlens-scan@main
  with:
...
include:
  - remote: 'https://raw.githubusercontent.com/breachlens-dev/breachlens/main/ci-templates/gitlab/breachlens-scan.gitlab-ci.yml'
variables:
...
// Full template: ci-templates/jenkins/Jenkinsfile
pipeline {
