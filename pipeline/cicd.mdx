...
If your org has a **default policy** (Settings → Policies), the severity-gate picker shows whether the file matches it and offers a one-click align, so generated pipelines stay consistent with org-wide intent. A pipeline file's strictest gate is HIGH — for a stricter policy, the GitHub App enforces it in full on pull requests.
