...
<Steps>
  ...
  <Step title="Set your license">
    ...
    <Note>
      The license is verified at api startup. Tampering with it (or running without one) prevents the api from booting. See the [License CLI](/cli#license-cli) to apply or rotate a license.
    </Note>
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Configure SSO & MFA" icon="key" href="/auth">
    Wire Microsoft Entra ID, Okta, Auth0, Google Workspace, or Keycloak. Multi-IdP per org supported.
  </Card>
  <Card title="Report on compliance" icon="clipboard-check" href="/compliance">
    Map findings to SOC 2, PCI DSS, and the OWASP catalogs, then export an auditor-ready evidence pack.
  </Card>
  <Card title="Fit it into CI/CD" icon="diagram-project" href="/pipeline/cicd">
    Gate pull requests and pipelines on new Critical/High findings with the BreachLens CLI.
  </Card>
</CardGroup>
