...
Applying, verifying, and issuing BreachLens licenses — including the vendor-side
`scripts/license/issue-license.ts` pipeline — has its own page.

<Card title="Licensing" icon="key" href="/deployment/licensing">
  Apply a license (operator), understand what the runtime check does, and issue one (vendor).
</Card>
