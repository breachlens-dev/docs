...
<Note>
  **Credentials are supplied as keys today** — an IAM access key, an Azure client secret, or a GCP service-account key. Keyless onboarding (AWS role assumption, GCP Workload Identity Federation, Azure managed identity) is on the roadmap; [tell us](mailto:hi@breachlens.app?subject=BreachLens%20keyless%20cloud%20onboarding) if it's a procurement gate. Rotate the key on your side and re-paste it any time from the account's Settings.
</Note>
